...
if len(sys.argv) < 2:
    # standard starting threshold value (used to be a good compromise) -> can easily be smaller for pictures without white border
    thresh = 220
...
files_by_type = []
types = ('*.bmp', '*.BMP', '*.tiff', '*.TIFF', '*.tif', '*.TIF', '*.jpg', '*.JPG',
         '*.JPEG', '*.jpeg', '*.png', '*.PNG', )  # all should work but only .jpg was tested
for t in types:
    if glob.glob(t) != []:
        files_by_type.append(glob.glob(t))
for f in files_by_type:
    for fil in f:
        main(thresh, crop, fil)
